...
def convert_input_to_restaurant(line):
    ...
    town_check = address[1].split(',')
    if town_check[0] == 'Troy' and restaurant_type.find('American') != -1:
        name = m[0]
        latitude = float(m[1])
        longitude = float(m[2])
        url = m[4]
...
if __name__ == "__main__":
    ...
    for i in range(num_restaurants):
        if restaurants[i] != -1 and restaurants[i].average_review() > 3:
            printlist.append(str(restaurants[i].name))
            printlist.sort()
