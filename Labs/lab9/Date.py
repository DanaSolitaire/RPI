'''
Start to the Date class for Lab 9.  This code will not run in Python
until three methods are added:
    __init__,
    __str__
    same_day_in_year
'''
class Date(object): 
    def __init__(self, year0 = 1900, month0 = 1, day0 = 1):
        self.year = year0
        self.month = month0
        self.day = day0
    def __str__(self):
        self.month = str(self.month).rjust(2,'0')
        self.day = str(self.day).rjust(2,'0')
        return '{}/{}/{}'.format(self.year,self.month,self.day)
    
    def same_day_in_year(self, other):
        return self.day == other.day and self.month == other.month
    def is_leap_year(self):
        return int(self.year) % 4 == 0 or int(self.year) % 100 == 0\
               and int(self.year) % 400 == 0  
    def __lt__(self, other):
        if int(self.year) < int(other.year):
            return True
        elif int(self.year) == int(other.year):
            if int(self.month) < int(other.month):
                return True 
            elif int(self.month) == int(other.month):
                if int(self.day) < int(other.day):
                    return True
        return False
        #return (int(self.year) < int(other.year) )or (int(self.month) < int(other.month) )or (int(self.day) < int(other.day))

days_in_month = [ 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 ]
month_names = [ '', 'January', 'February', 'March', 'April', 'May', 'June', 'July',\
                    'August','September', 'October', 'November', 'December' ]

if __name__ == "__main__":
    d1 = Date(1972, 3, 27)
    d2 = Date(1998, 4, 13)
    d3 = Date(1996, 4, 13)
    d41 = Date(1998, 4, 11)
    d31 = Date(1998, 5, 13)
    print("d1: " + str(d1))
    print("d2: " + str(d2))
    print("d3: " + str(d3))
    print("d1.same_day_in_year(d2)", d1.same_day_in_year(d2))
    print("d2.same_day_in_year(d3)", d2.same_day_in_year(d3))
    print('d1.is_leap_year()', d1.is_leap_year())
    print('d2.is_leap_year()', d2.is_leap_year())
    print ()
    print(d1 < d2)
    print(d2 < d31)
    print(d31 < d41)